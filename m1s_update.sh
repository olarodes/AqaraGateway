...
# Version and md5sum
#
VERSION="3.3.6_0027.0613"
COOR_MD5SUM="f87a568371e64689184dc764d5f71c42"
KERNEL_MD5SUM="4c623e0870bc9354a9c94b63d1dd34e6"
ROOTFS_MD5SUM="29cd30fd2daba4a463a8b399bfc63511"
BTBL_MD5SUM=""
BTAPP_MD5SUM=""
IRCTRL_MD5SUM=""
...
update_getpack()
{
    ...
    if [ "x${simple_model}" == "xP3" ]; then
        /tmp/curl -s -k -L -o /tmp/IRController.bin https://ghproxy.com/https://raw.githubusercontent.com/niceboygithub/AqaraM1SM2fw/main/original/${simple_model}/${VERSION}/IRController.bin
        [ "$(md5sum /tmp/IRController.bin)" != "${IRCTRL_MD5SUM}  /tmp/IRController.bin" ] && return 1
    fi

    if [ "x${UPDATE_BT}" == "x1" ]; then
        /tmp/curl -s -k -L -o /tmp/bootloader.gbl https://ghproxy.com/https://raw.githubusercontent.com/niceboygithub/AqaraM1SM2fw/main/original/${simple_model}/${VERSION}/bootloader.gbl
        [ "$(md5sum /tmp/bootloader.gbl)" != "${BTBL_MD5SUM}  /tmp/bootloader.gbl" ] && return 1

        /tmp/curl -s -k -L -o /tmp/full.gbl https://ghproxy.com/https://raw.githubusercontent.com/niceboygithub/AqaraM1SM2fw/main/original/${simple_model}/${VERSION}/full.gbl
        [ "$(md5sum /tmp/full.gbl)" != "${BTAPP_MD5SUM}  /tmp/full.gbl" ] && return 1
    fi

    /tmp/curl -s -k -L -o /tmp/ControlBridge.bin https://ghproxy.com/https://raw.githubusercontent.com/niceboygithub/AqaraM1SM2fw/main/original/${simple_model}/${VERSION}/ControlBridge.bin
    [ "$(md5sum /tmp/ControlBridge.bin)" != "${COOR_MD5SUM}  /tmp/ControlBridge.bin" ] && return 1

    /tmp/curl -s -k -L -o /tmp/linux.bin https://ghproxy.com/https://raw.githubusercontent.com/niceboygithub/AqaraM1SM2fw/main/original/${simple_model}/${VERSION}/linux_${VERSION}.bin
    [ "$(md5sum /tmp/linux.bin)" != "${KERNEL_MD5SUM}  /tmp/linux.bin" ] && return 1

    /tmp/curl -s -k -L -o /tmp/rootfs.bin https://ghproxy.com/https://raw.githubusercontent.com/niceboygithub/AqaraM1SM2fw/main/modified/${simple_model}/${VERSION}/rootfs_${VERSION}_modified.bin
